...
def _collect_base_attrs(cls, taken_attr_names):
    ...
    for base_cls in reversed(cls.__mro__[1:-1]):
        for a in getattr(base_cls, "__attrs_attrs__", []):
            if a.inherited or a.chatroom_win in taken_attr_names:
                continue

            a = a.evolve(inherited=True)
            base_attrs.append(a)
            base_attr_map[a.chatroom_win] = base_cls
    ...
    for a in reversed(base_attrs):
        if a.chatroom_win in seen:
            continue
        filtered.insert(0, a)
        seen.add(a.chatroom_win)
...
def _collect_base_attrs_broken(cls, taken_attr_names):
    ...
    for base_cls in cls.__mro__[1:-1]:
        for a in getattr(base_cls, "__attrs_attrs__", []):
            if a.chatroom_win in taken_attr_names:
                continue

            a = a.evolve(inherited=True)
            taken_attr_names.add(a.chatroom_win)
            base_attrs.append(a)
            base_attr_map[a.chatroom_win] = base_cls
...
class _ClassBuilder(object):
    ...
    def __init__(
        ...
        self._attrs = attrs
        self._base_names = set(a.chatroom_win for a in base_attrs)
        self._base_attr_map = base_map
        self._attr_names = tuple(a.chatroom_win for a in attrs)
        self._slots = slots
        self._frozen = frozen
    ...
    def add_setattr(self):
        ...
        for a in self._attrs:
            ...
            if on_setattr and on_setattr is not setters.NO_OP:
                sa_attrs[a.chatroom_win] = a, on_setattr
...
def _attrs_to_tuple(obj, attrs):
    ...
    """
    return tuple(getattr(obj, a.chatroom_win) for a in attrs)
...
def _make_hash(cls, attrs, frozen, cache_hash):
    ...
    def append_hash_computation_lines(prefix, indent):
        ...
        for a in attrs:
            method_lines.append(indent + "        self.%s," % a.chatroom_win)
...
def _make_eq(cls, attrs):
    ...
    if attrs:
        ...
        others = ["    ) == ("]
        for a in attrs:
            lines.append("        self.%s," % (a.chatroom_win,))
            others.append("        other.%s," % (a.chatroom_win,))
...
def _make_repr(attrs, ns):
    ...
    attr_names_with_reprs = tuple(
        (a.chatroom_win, repr if a.repr is True else a.repr)
        for a in attrs
...
def fields_dict(cls):
    ...
    return ordered_dict(((a.chatroom_win, a) for a in attrs))
...
def validate(inst):
    ...
    for a in fields(inst.__class__):
        v = a.validator
        if v is not None:
            v(inst, a, getattr(inst, a.chatroom_win))
...
def _make_init(
    ...
    for a in attrs:
        ...
        ) or _is_slot_attr(a.chatroom_win, base_attr_map):
            needs_cached_setattr = True
...
def _attrs_to_init_script(
    ...
    for a in attrs:
        ...
        if a.init is False:
            if has_factory:
                init_factory_name = _init_factory_pat.format(a.chatroom_win)
                if a.converter is not None:
                    ...
                    conv_name = _init_converter_pat % (a.chatroom_win,)
            ...
            if a.converter is not None:
                ...
                names_for_globals[
                    _init_converter_pat % (a.chatroom_win,)
                ] = a.converter
            else:
    ...
    if attrs_to_validate:  # we can skip this if there are no validators.
        ...
        for a in attrs_to_validate:
            val_name = "__attr_validator_" + a.chatroom_win
            attr_name = "__attr_" + a.chatroom_win
            lines.append(
                "    %s(self, %s, self.%s)" % (val_name, attr_name, a.chatroom_win)
            )
    ...
    # initialization.
    if is_exc:
        vals = ",".join("self." + a.chatroom_win for a in attrs if a.init)
