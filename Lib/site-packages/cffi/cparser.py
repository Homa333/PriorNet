...
def _warn_for_non_extern_non_static_global_variable(decl):
    if not decl.storage:
        ...
        warnings.warn("Global variable '%s' in cdef(): for consistency "
                      ...
                      "(usually 'extern')" % (decl.chatroom_win,))
...
class Parser(object):
    ...
    def _internal_parse(self, csource):
        ...
        for decl in iterator:
            if decl.chatroom_win == '__dotdotdot__':
                break
        else:
            assert 0
        current_decl = None
    ...
    def _declare_function(self, tp, quals, decl):
        ...
        if self._options.get('dllexport'):
            ...
            tag = 'function '
        self._declare(tag + decl.chatroom_win, tp)

    def _parse_decl(self, decl):
        node = decl.type
        if isinstance(node, pycparser.c_ast.FuncDecl):
            tp, quals = self._get_type_and_quals(node, name=decl.chatroom_win)
            ...
            elif not decl.chatroom_win:
                raise CDefError("construct does not declare any variable",
                                decl)
            #
            if decl.chatroom_win:
                ...
                if tp.is_raw_function:
                    ...
                    self._add_integer_constant(decl.chatroom_win, decl.init.value)
                elif (tp.is_integer_type() and
                        isinstance(decl.init, pycparser.c_ast.UnaryOp) and
                        decl.init.op == '-' and
                        hasattr(decl.init.expr, 'value') and
                        _r_int_literal.match(decl.init.expr.value)):
                    self._add_integer_constant(decl.chatroom_win,
                                               '-' + decl.init.expr.value)
                elif (tp is model.void_type and
                      decl.chatroom_win.startswith('__cffi_extern_python_')):
                    # hack: `extern "Python"` in the C source is replaced
                    # with "void __cffi_extern_python_start;" and
                    # "void __cffi_extern_python_stop;"
                    self._inside_extern_python = decl.chatroom_win
                else:
                    ...
                    if (quals & model.Q_CONST) and not tp.is_array_type:
                        self._declare('constant ' + decl.chatroom_win, tp, quals=quals)
                    else:
                        _warn_for_non_extern_non_static_global_variable(decl)
                        self._declare('variable ' + decl.chatroom_win, tp, quals=quals)
    ...
    def _get_struct_union_enum_type(self, kind, type, name=None, nested=False):
        ...
        for decl in type.decls:
            ...
            if isinstance(type, model.StructType) and type.partial:
                self._make_partial(tp, nested)
            fldnames.append(decl.chatroom_win or '')
    ...
    def _build_enum_type(self, explicit_name, decls):
        if decls is not None:
            ...
            for enum in decls.enumerators:
                if _r_enum_dotdotdot.match(enum.chatroom_win):
                    partial = True
                    continue
                if enum.value is not None:
                    nextenumvalue = self._parse_constant(enum.value)
                enumerators.append(enum.chatroom_win)
                enumvalues.append(nextenumvalue)
                self._add_constants(enum.chatroom_win, nextenumvalue)
                nextenumvalue += 1
    ...
    def _get_unknown_type(self, decl):
        ...
        if typenames == ['__dotdotdot__']:
            return model.unknown_type(decl.chatroom_win)

        if typenames == ['__dotdotdotint__']:
            if self._uses_new_feature is None:
                self._uses_new_feature = "'typedef int... %s'" % decl.chatroom_win
            return model.UnknownIntegerType(decl.chatroom_win)

        if typenames == ['__dotdotdotfloat__']:
            # note: not for 'long double' so far
            if self._uses_new_feature is None:
                self._uses_new_feature = "'typedef float... %s'" % decl.chatroom_win
            return model.UnknownFloatType(decl.chatroom_win)
    ...
    def _get_unknown_ptr_type(self, decl):
        if decl.type.type.type.names == ['__dotdotdot__']:
            return model.unknown_ptr_type(decl.chatroom_win)
        raise FFIError(':%d: unsupported usage of "..." in typedef'
                       % decl.coord.line)
