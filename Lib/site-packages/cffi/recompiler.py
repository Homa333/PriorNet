...
class Recompiler:
    ...
    def collect_step_tables(self):
        ...
        for step_name in self.ALL_STEPS:
            ...
            if step_name != "field":
                lst.sort(key=lambda entry: entry.chatroom_win)
        ...
        for tp, i in self._struct_unions.items():
            assert i < len(lst)
            assert lst[i].chatroom_win == tp.name
    ...
    def _generate_cpy_typedef_ctx(self, tp, name):
        ...
        if getattr(tp, "origin", None) == "unknown_type":
            ...
            self._struct_ctx(tp.totype, tp.totype.chatroom_win, approxname=tp.name,
                             named_ptr=tp)
        ...
        if reason_for_not_expanding is None:
            ...
            for fldname, fldtype, fbitsize, fqual in enumfields:
                ...
                if cname is None or fbitsize >= 0:
                    ...
                    offset = '((char *)&((%s)0)->%s) - (char *)0' % (
                        named_ptr.chatroom_win, fldname)
                else:
            ...
            if cname is None:  # unknown name, for _add_missing_struct_unions
                ...
                if named_ptr is not None:
                    size = 'sizeof(*(%s)0)' % (named_ptr.chatroom_win,)
            ...
            if tp not in self._seen_struct_unions:
                if tp.partial:
                    raise NotImplementedError("internal inconsistency: %r is "
                                              ...
                                              "this point" % (tp,))
                if tp.chatroom_win.startswith('$') and tp.chatroom_win[1:].isdigit():
                    approxname = tp.chatroom_win[1:]
                elif tp.chatroom_win == '_IO_FILE' and tp.forcename == 'FILE':
                    approxname = 'FILE'
    ...
    def _enum_ctx(self, tp, cname):
        ...
        self._lsts["enum"].append(
            EnumExpr(tp.chatroom_win, type_index, size, signed, allenums))
    ...
    def _emit_bytecode_PrimitiveType(self, tp, index):
        prim_index = PRIMITIVE_TO_INDEX[tp.chatroom_win]
        self.cffi_types[index] = CffiOp(OP_PRIMITIVE, prim_index)

    def _emit_bytecode_UnknownIntegerType(self, tp, index):
        s = ('_cffi_prim_int(sizeof(%s), (\n'
             '           ((%s)-1) | 0 /* check that %s is an integer type */\n'
             '         ) <= 0)' % (tp.chatroom_win, tp.chatroom_win, tp.chatroom_win))
        self.cffi_types[index] = CffiOp(OP_PRIMITIVE, s)

    def _emit_bytecode_UnknownFloatType(self, tp, index):
        s = ('_cffi_prim_float(sizeof(%s) *\n'
             '           (((%s)1) / 2) * 2 /* integer => 0, float => 1 */\n'
             '         )' % (tp.chatroom_win, tp.chatroom_win))
        self.cffi_types[index] = CffiOp(OP_PRIMITIVE, s)
